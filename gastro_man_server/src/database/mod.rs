...
const PREPARE_ERROR: &str = "dbm > Error while preparing statement";
...
impl DBManager {
  pub fn new() -> Result<DBManager, sqlite::Error> {
    ...
    if missing_tables.contains(&"users") || usr_count <= 0 {
      ...
      let password: String = std::time::SystemTime::now()
      ...
      match self.db_query.1.recv() {
        Ok(req) => {
          match req {
            ...
            DBRequest::PermissionsGetRequest(sid, answer) => {
              self.handle_permissionsget(sid, answer)
            }
          }
        }
  ...
  fn handle_permissionsget(
    &mut self,
    sid: String,
    answer: Producer<requests::PermissionsGetResponse>,
  ) {
    //get user from sid
    match self.sessions.get(dbg!(&sid)) {
      Some(uid) => {
        let mut stmt = self.con.prepare(format!("SELECT perm_flags FROM users WHERE id = '{}' LIMIT 1", uid)).expect(PREPARE_ERROR);
        if let sqlite::State::Row = stmt.next().unwrap() {
          let flag_raw: i64 = stmt.read(0).expect("Can't read perm_flags");
          answer.push(Some(UserPermissionFlags(flag_raw)));
        }
        else {
          answer.push(None);
        }
      },
      None => answer.push(None),
    }
  }
  ...
  fn handle_user_get(&mut self, user: String, answer: Producer<Option<User>>) {
    let mut stmt = self
      .con
      .prepare(format!("SELECT * FROM users WHERE user_name = '{}'", user))
      .expect(PREPARE_ERROR);
    if let sqlite::State::Row = stmt.next().unwrap() {
        ...
        Ok(usr) => {
          self.sessions.insert(
            usr
              .session_id
              .clone()
              .expect("User::from_db did not set a session_id"),
            usr.id.expect("User::from_db did not set the id"),
          );
